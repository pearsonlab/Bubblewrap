...
class RBFN:
    ...
    def step(self, x, loop=3, _mask=None):
        for _ in range(loop):
            value, grads = self._obj(self.ker, x, self.params, mask=_mask)
            self.opt_state = self.opt_update(self.t, grads, self.opt_state)
        self.t += 1
        return value
    
    def step_online(self, x, loop=3):
        """
        Assuming the role for splitting up data for online learning for now.
        Pad for t=0...self.window.
        """
        self.mask = index_update(self.mask, self.t - 1, 1.)
        ...
        return self.step(z, loop, self.mask)
    ...
    @staticmethod
    def _mse(ker: Callable, x: DeviceArray, p: dict[str, DeviceArray], mask: Optional[DeviceArray]=None):
        """||g(x_{t-1}) + x_{t-1} - x_t||²"""
        if mask is None:
